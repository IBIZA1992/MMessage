...
@interface DJChatViewController () <UITableViewDelegate, UITableViewDataSource, JMessageDelegate, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVAudioRecorderDelegate,AVAudioPlayerDelegate, UIGestureRecognizerDelegate>
...
@property(nonatomic, strong) UIPanGestureRecognizer *removeSelfView;
...
- (void)keyboardWillShow:(NSNotification *)Notification {
    ...
    _removeSelfView = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(removeSelfView:)];
    [_tableview addGestureRecognizer:_removeSelfView];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    //群聊会话
    if(_single.messageType == 2) {
        [JMSGConversation createGroupConversationWithGroupId:_single.groupID completionHandler:^(id resultObject, NSError *error) {
            NSString *str = self->_textview.textfield.text;
            JMSGTextContent  *content = [[JMSGTextContent alloc] initWithText:str];
            JMSGMessage *message = [JMSGMessage createGroupMessageWithContent:content groupId:self->_single.groupID];
            [JMSGMessage sendMessage:message];
            /**获取列表的所有消息*/
            [resultObject allMessages:^(id resultObject, NSError *error) {
                self->_single.messageArray = @[].mutableCopy;
...
- (void)playvoice:(NSNotification *)info {
    ...
    [voicecontent voiceData:^(NSData *data, NSString *objectId, NSError *error) {
        ...
        [self->_player play];
        __unused NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:self->_player.duration target:self selector:@selector(stop) userInfo:nil repeats:YES];
    }];
}
///播放结束
- (void)stop {
...
- (void)onReceiveMessage:(JMSGMessage *)message error:(NSError *)error {
    ...
    //如果为群聊
    if(_single.messageType == 2) {
        [[JMSGConversation groupConversationWithGroupId:_single.groupID] allMessages:^(id resultObject, NSError *error) {
            self->_single.messageArray = @[].mutableCopy;
            self->_single.messageArray = (NSMutableArray *)resultObject;
            [self->_tableview reloadData];
            [self->_tableview scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:self->_single.messageArray.count - 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
