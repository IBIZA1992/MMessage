...
- (void)SetData:(JMSGMessage *)message {
    ...
    NSString *imagepath =[path_document stringByAppendingString:message.fromUser.username];/**将头像名为地址名存入*/
    if([_single.imagePathArray containsObject:imagepath] == YES) {/**如果在本地已经存储头像*/
        UIImage *image = [UIImage imageWithContentsOfFile:imagepath]; /**直接本地获取数据*/
        _profile_image_url.image = image;
    }
    else {/**如果本地没有存储头像*/
        if(message.fromUser.avatar){ /**如果该用户有头像*/
            ...
            [JMSGUser userInfoArrayWithUsernameArray:array completionHandler:^(id resultObject, NSError *error) { /**通过对方的userdata获取对方的信息*/
                ...
                [user thumbAvatarData:^(NSData *data, NSString *objectId, NSError *error) { /**从网络请求头像数据*/
                    self->_profile_image_url.image = [UIImage imageWithData:data]; /**更新UI数据*/
                    ...
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{/**在子线程中将头像数据存入本地磁盘*/
                        [UIImagePNGRepresentation(self->_profile_image_url.image) writeToFile:imagepath atomically:YES];
                    });
                }];
            }];
        }
    ...
    if(message.contentType == kJMSGContentTypeImage){/**如果该信息为图片类型*/
       JMSGImageContent *imagecontent = (JMSGImageContent *)contentimage;
        ...
        NSString *path_document = NSHomeDirectory();
        NSString *imagepath =[path_document stringByAppendingString:[message.timestamp stringValue]];/**将该图片的时间戳为地址名存入*/
        /**如果该图片本地已经存储，直接本地加载*/
